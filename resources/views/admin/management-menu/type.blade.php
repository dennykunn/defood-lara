...
@section('breadcrumb-items')
    ...
    <p>Management Menu</p>
    <p>/</p>
    <a href="{{ route('type-list') }}" class="block text-yellow-dark font-semibold">Type</a>
@endsection
...
@section('content')
    <div class="grid lg:grid-cols-4 md:grid-cols-3 xs:grid-cols-2 grid-cols-1 gap-4">
            ...
            <div class="relative w-full max-w-md max-h-full">
                <div class="relative bg-white rounded-lg shadow">
                    <div class="px-6 py-6 lg:px-8">
                        <h3 class="mb-4 text-xl text-blue-darker font-bold">Create new type</h3>

                        <form class="space-y-6" action="{{ route('type-add') }}" method="POST"
                            enctype="multipart/form-data">
                            @csrf
                            <div>
                                <label for="name" class="block mb-2 text-sm font-medium text-blue-darker">Type</label>
        ...
        @foreach ($types as $type)
            <div class="col-span-1 w-full rounded-xl bg-white shadow-cmsCard group overflow-hidden">
                <div class="overflow-hidden w-full h-36">
                    <img src="{{ Storage::url($type->image) }}" alt="{{ $type->slug }}"
                        class="w-full object-cover object-center group-hover:scale-125 transition-all duration-500">
                </div>

                <div class="flex items-center justify-center h-20 relative">
                    <p class="font-bold text-xl capitalize text-blue-darker">{{ $type->name }}</p>

                    <div
                        class="absolute top-full left-0 bg-yellow-dark/50 w-full h-full group-hover:w-full group-hover:top-0 invisible group-hover:visible transition-all duration-500 flex items-center justify-center gap-2">
                        <div type="button" id="btnEdit-{{ $type->id }}"
                            class="bg-white h-0 w-0 group-hover:h-10 group-hover:w-10 rounded text-yellow-dark hover:bg-blue-600 hover:text-white overflow-hidden flex items-center justify-center transition-all duration-300 cursor-pointer">
                            <i data-feather="edit"></i>
                        </div>
                        <button type="button" id="btnDelete-{{ $type->id }}"
                    ...
                    <div id="popup-edit-{{ $type->id }}"
                        ...
                        <div class="relative w-full max-w-md max-h-full">
                            <div class="relative bg-white rounded-lg shadow">
                                <div class="px-6 py-6 lg:px-8">
                                    <h3 class="mb-4 text-xl text-blue-darker font-bold">Edit {{ $type->name }} type</h3>

                                    <form class="space-y-6" action="{{ route('type-edit', ['id' => $type->id]) }}"
                                        ...
                                        <div class="">
                                            <label for="images" class="drop-container mt-2" id="dropcontainer">
                                                ...
                                                <div class="flex sm:flex-row flex-col sm:items-center items-center gap-2">
                                                    ...
                                                    <img id="imageDisplay" src="{{ Storage::url($type->image) }}"
                                                        alt="{{ $type->name }}"
                            ...
                            <div class="relative bg-white rounded-lg shadow">
                                <form action="{{ route('type-delete', ['id' => $type->id]) }}" method="POST"
                                    ...
                                    <h3 class="mb-5 text-lg font-normal text-gray-500">Are
                                        you sure you want to delete <span class="text-red-500">{{ $type->name }}</span>
                                        type ?</h3>
                                    <button type="submit"
