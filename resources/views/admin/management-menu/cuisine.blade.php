...
@section('breadcrumb-items')
    ...
    <p>Management Menu</p>
    <p>/</p>
    <a href="{{ route('cuisine-list') }}" class="block text-yellow-dark font-semibold">Cuisine</a>
@endsection
...
@section('content')
    <div class="grid lg:grid-cols-4 md:grid-cols-3 xs:grid-cols-2 grid-cols-1 gap-4">
            ...
            <div class="relative w-full max-w-md max-h-full">
                <div class="relative bg-white rounded-lg shadow">
                    <div class="px-6 py-6 lg:px-8">
                        <h3 class="mb-4 text-xl text-blue-darker font-bold">Create new cuisine</h3>

                        <form class="space-y-4" action="{{ route('cuisine-add') }}" method="POST"
                            enctype="multipart/form-data">
                            @csrf
                            <div>
                                <label for="name"
                                    class="block mb-2 text-sm font-medium text-blue-darker">Cuisine</label>
        ...
        @foreach ($cuisines as $cuisine)
            <div class="col-span-1 w-full rounded-xl bg-white relative group overflow-hidden shadow-cmsCard">

                <div class="overflow-hidden w-full h-36">
                    <img src="{{ Storage::url($cuisine->image) }}" alt="{{ $cuisine->slug }}"
                        class="w-full h-full object-cover object-center group-hover:scale-125 transition-all duration-500">
                </div>

                <div class="flex items-center justify-center h-20 relative">
                    <p class="font-bold text-xl capitalize text-blue-darker">{{ $cuisine->name }}</p>
                    ...
                    <div
                        ...
                        <div type="button" id="btnEdit-{{ $cuisine->id }}"
                        ...
                        <button type="button" id="btnDelete-{{ $cuisine->id }}"
                    ...
                    <div id="popup-edit-{{ $cuisine->id }}"
                        ...
                        <div class="relative w-full max-w-md max-h-full">
                            <div class="relative bg-white rounded-lg shadow">
                                <div class="px-6 py-6 lg:px-8">
                                    <h3 class="mb-4 text-xl text-blue-darker font-bold">Edit {{ $cuisine->name }} cuisine
                                    </h3>

                                    <form class="space-y-6" action="{{ route('cuisine-edit', ['id' => $cuisine->id]) }}"
                                        ...
                                        <div class="">
                                            <label for="images" class="drop-container mt-2" id="dropcontainer">
                                                ...
                                                <div class="flex sm:flex-row flex-col sm:items-center items-center gap-2">
                                                    ...
                                                    <img id="imageDisplay" src="{{ Storage::url($cuisine->image) }}"
                                                        alt="{{ $cuisine->name }}"
                    ...
                    {{-- Pop Up Delete --}}
                    <div id="popup-delete-{{ $cuisine->id }}"
                        class="hidden fixed top-0 left-0 right-0 z-50 p-4 overflow-x-hidden overflow-y-auto h-screen max-h-full items-center justify-center bg-black/20">
                        <div class="relative w-full max-w-md max-h-full">
                            <div class="relative bg-white rounded-lg shadow">
                                <form action="{{ route('cuisine-delete', ['id' => $cuisine->id]) }}" method="POST"
                                    ...
                                    <h3 class="mb-5 text-lg font-normal text-gray-500">Are
                                        you sure you want to delete <span class="text-red-500">{{ $cuisine->name }}</span>
                                        cuisine ?</h3>
                                    <button type="submit"
